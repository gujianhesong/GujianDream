...
//    }
	...
	std::string nickName = PlayerController::sharePlayerController()->getPlayerName();
	...
	m_BloodTTF = dynamic_cast<ui::Text*>(ui::Helper::seekWidgetByName(pNode,"Label_Blood"));
	m_BloodTTF->setString(StringUtils::format("%d/%d", Player::sharePlayer()->getBlood(), Player::sharePlayer()->getBloodCap()));
	...
	m_magicTTF = dynamic_cast<ui::Text*>(ui::Helper::seekWidgetByName(pNode,"Label_Magic"));
	m_magicTTF->setString(StringUtils::format("%d/%d", Player::sharePlayer()->getMagic(), Player::sharePlayer()->getMagicCap()));
...
void GameInfoUIController::modeSwitch(cocos2d::Ref *sender, Widget::TouchEventType type)
{
    ...
    if (m_isShowTabBar)
    {
		...
		CallFunc* finish = CallFunc::create(CC_CALLBACK_0(GameTabBarMenu::showGameTabBarMenu, m_gameTabBarMenu));
...
{
    m_magicPro->setPercent(100*Player::sharePlayer()->getMagic()/Player::sharePlayer()->getMagicCap());
	m_magicTTF->setString(StringUtils::format("%d/%d", Player::sharePlayer()->getMagic(), Player::sharePlayer()->getMagicCap()));
}
