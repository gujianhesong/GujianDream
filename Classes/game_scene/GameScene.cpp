...
void GameScene::didChangeDirectionTo(SimpleDPad *simpleDPad, cocos2d::Point direction)
{
...
void GameScene::isHoldingDirection(SimpleDPad *simpleDPad, cocos2d::Point direction)
{
...
void GameScene::playMapMusic(int mapId)
{
	auto path = CCString::createWithFormat("music/map_music/%d.mp3", mapId)->getCString();
	if (FileUtils::getInstance()->isFileExist(path))
    {
		CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic(true);
		//CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic(path, true);
...
void GameScene::updateImageDisplay(cocos2d::Rect &showRect, cocos2d::Rect &hideRect)
{
...
//    printf("111111\n");
    ...
    do{
        ...
        if (this->getSelected() == NULL){
			if(! m_bgMap->isTouchNpcAndNotNeedMoveToNpc(point)){
				//没有选中任何个体，则移动到点击处
...
void GameScene::addEnemy()
{
	...
	enemy->setAnchorPoint(Vec2::ANCHOR_MIDDLE_BOTTOM);
	...
	int index = CsvUtil::getInstance()->findValueInWithLine(StringUtils::format("%d", number), en_monster_id, CSVFILE_MONSTER);


	enemy->setNickName(CsvUtil::getInstance()->getText(index, en_monster_name, CSVFILE_MONSTER));
	enemy->setBlood(CsvUtil::getInstance()->getInt(index, en_monster_hp, CSVFILE_MONSTER));
    enemy->setBloodCap(enemy->getBlood());
	enemy->setMagic(CsvUtil::getInstance()->getInt(index, en_monster_mp, CSVFILE_MONSTER));
	enemy->setMagicCap(enemy->getMagic());
	enemy->setTheAttack(CsvUtil::getInstance()->getInt(index, en_monster_attack, CSVFILE_MONSTER));
	enemy->setActive(CsvUtil::getInstance()->getInt(index, en_monster_active, CSVFILE_MONSTER));
	enemy->setCanMoved(CsvUtil::getInstance()->getInt(index, en_monster_can_move, CSVFILE_MONSTER));
