...
#define FIGURE_ROUTE "texture_figure_new/"
#define MONSTER_ROUTE "texture_monster_new/"
#define HAIR_ROUTE "texture_figure/"
#define WEAPON_ROUTE "texture_weapon_new/"
#define SKILL_ROUTE "texture_skill/"
#define OTHER_ROUTE "texture_set/"
#define NPC_ROUTE "texture_npc_new/"
    ...
    static void addSpriteFramesAsync(string& path, Ref *target = NULL, SEL_CallFunc selector = NULL)
    {
        ...
        if (itr == textureMap.end())
        {
            if (CC_TARGET_PLATFORM == CC_PLATFORM_MAC || GAME_SCENE == NULL)
            {
                ...
                log("texture::---111-AddPath=%s", completePath.c_str());
            }
            else
            {
...
string TextureController::getTextureRoute(TexturePathType type, unsigned int number)
{
    ...
    switch (type)
    {
		...
		case TexturePathNPC:
        {
            path = string(NPC_ROUTE) + NPC_PATH;	
            break;
        }
    ...
    if (path.compare("") != 0)
    {			
		path = StringUtils::format("%s%d", path.c_str(), number);
...
void TextureController::addSpriteFrames(string path, Ref *target, SEL_CallFunc selector)
{
	do
	{
		CC_BREAK_IF(path.compare("") == 0);
