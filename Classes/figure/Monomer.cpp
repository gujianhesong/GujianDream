...
string Monomer::getNickName(){
	return m_nickName;
}

void Monomer::setNickName(string nickName){
	m_nickName = nickName;
...
void Monomer::showFigure()
{
    do
    {
		...
		m_nMonomer->setAnchorPoint(Vec2::ANCHOR_MIDDLE_BOTTOM);
		m_nMonomer->setPosition(Vec2(this->getContentSize().width/2, 0));
        
        this->addChild(m_nMonomer, 1);
		...
		//m_bg->setOpacity(128);
		//m_bg->setContentSize(Size(m_nMonomer->getContentSize()));
		//m_bg->setPosition(this->getContentSize()/2);
		//this->addChild(m_bg);

		//血条
        Point bloodPoint = Point(this->getContentSize().width * this->getAnchorPoint().x, m_nMonomer->getContentSize().height + 10);
        
        m_bloodBg = Sprite::create("blood_box.png");
        m_bloodBg->setAnchorPoint(Vec2(0.5, 0));
        m_bloodBg->setPosition(bloodPoint);
        this->addChild(m_bloodBg);
        m_bloodBg->setScale(0.3);
        
        bloodIn = Sprite::create("blood_in.png");
        bloodIn->setAnchorPoint(Vec2::ZERO);
        m_bloodBg->addChild(bloodIn);
		...
		label_nickname->setString(m_nickName);
...
void Monomer::hideFigure()
{
    do
    {
        ...
        m_bloodBg->removeFromParent();
		m_bloodBg = NULL;

		label_nickname->removeFromParent();
		label_nickname = NULL;
...
bool Monomer::isInTheFieldOfView(Monomer *var)
{
    ...
    {
		log("isInTheFieldOfView------%d,%d,%d,%d",point.x, point.z, varPoint.x, varPoint.z);
		log("isInTheFieldOfView------%f,%d",point.getDistance(varPoint), m_visualRange);
        return true;
    }
...
CCArray* Monomer::actionsWithPoint(MapPoint& pStart, MapPoint& pFinish)
{
	...
	float time = 0.3f * pStart.getDistanceFloat(pFinish) / m_runSpeed;
...
void Monomer::updateBloodProgress()
{
    do
    {
        CC_BREAK_IF(m_bloodBg == NULL);
        
        Rect rect = Rect::ZERO;
        rect.size = Size(m_bloodBg->getContentSize().width*m_blood/m_bloodCap, bloodIn->getContentSize().height);
...
void Monomer::onContentSizeChanged()
{
	float beforeHeight = this->getContentSize().height;
	this->setContentSize(m_nMonomer->getContentSize());
	//m_bg->setContentSize(m_nMonomer->getContentSize());

	float afterHeight = this->getContentSize().height;

	if(m_bloodBg){
		m_bloodBg->setPosition(m_bloodBg->getPositionX(), m_bloodBg->getPositionY() + (afterHeight - beforeHeight));
	}
