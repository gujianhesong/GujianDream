...
private:
    ...
    LabelTTF* pullShowLabel;
    
    LabelTTF* waitingSprite;
...
public:
    ...
    void inWaiting()
    {
        ...
        this->inWaitingActions(0.05f);
        
    }
    
    void inWaitingActions(float t)
    {
...
bool LyfTableView::initWithTableView(float contentSizeHeight){
    ...
    if (!LayerColor::initWithColor(ccc4(255, 255, 255, 0)))
    {
        return false;
    }
...
{
    this->setContentSize(Size(this->getContentSize().width, contentSizeHeight));
    m_backNode->setPosition(Point(0, contentSizeHeight));
...
void LyfTableView::inertia(float t){
...
void LyfTableView::readDataInTurnLastPage(float t){
...
void LyfTableView::readDataInTurnNextpage(float t){
    
    this->reLoadData();
    m_backNode->setPosition(Point::ZERO);
    CallFunc* callFunc=CallFunc::create(this, callfunc_selector(LyfTableView::PullShowHideFalse));
...
void LyfTableView::cellGoHome(float t){
...
void LyfTableView::updateTableViewCellPos(){
    ...
    if (m_TableViewCells.size()>0 && this->getContentSize().width==0)
    {
        ...
        this->setContentSize(Size(cell->getContentSize().width, this->getContentSize().height));
    }
...
//void LyfTableView::visit()
//{
//    
//    glEnable(GL_SCISSOR_TEST);// 开启显示指定区域
//    
//    Point point = this->convertToWorldSpace(Vec2::ZERO);
...
//#endif
//
//    CCRect rect=CCRect(point.x * glScaleX + off_X,
